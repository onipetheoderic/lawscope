...
class DeskTopReadingPage extends React.Component {
   constructor() {
    super();
      this.state = {
        prevable: false,
...
chapterPages = (chapter_id) => {
  ...
  <div style={{fontWeight: '600', fontSize: 12}}>
    ...
    {this.state.chapterList.map((d, i) =>
      <li onClick={()=> this.chapterPages(d.id)} key={d.id} style={{cursor: 'pointer', paddingTop:6, paddingBottom:6, paddingLeft:3, boxShadow:"0 4px 8px 0 rgba(0,0,0,0.2)"}}>Chapter {i+1}</li>
      )}
    </ul>
  </div>
  ...
  <div style={{paddingLeft: 40, paddingTop: 40}}>
     ...
     <div style={{paddingTop: 5}}>
      ...
      <div style={{ width: '55%',  marginRight: '2%', transition: '0.3s', background: 'white', float: 'left',
  boxShadow:"0 4px 8px 0 rgba(0,0,0,0.2)"}}>
    <p style={{paddingTop: 20, textAlign: "center", justifyContent: 'center', fontWeight: '600', fontSize: 8}}>{this.state.currentPageNumber}</p>

    <p style={{padding:20, textAlign: 'center', lineHeight: 2.4}}>{this.state.currentPageContent}</p>
     {this.state.nextable &&
...
DeskTopReadingPage.propTypes = {
  title: PropTypes.string,
  description: PropTypes.string,
  photo: PropTypes.string
};
export default DeskTopReadingPage
